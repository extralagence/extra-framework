			...
			source: function(req, callback){
				$.ajax({
					...
					success: function(data){
						...
						//process response
						$.each(data, function(i, val){
							suggestions.push({"slug": val.slug, "taxonomy": val.taxonomy, "highlight_title": accent_folded_hilite(val.name, req.term), "title": val.name, "type": val.taxonomy_name, "url": val.url});
						});
						//pass array to callback
						callback(suggestions);
					}
